...
function draw() {
  ...
  switch (G.scr) {
    case 'title':
      try { dTitle(); } catch(e) { console.error('[draw title]', e.message); }
      // Diag: texto pequeño en esquina
      cx.fillStyle = '#0F0';
      cx.font = '8px "Press Start 2P"';
      cx.textAlign = 'right';
      cx.fillText('TITLE-FR:'+(G.tFr||0), 630, 20);
      cx.textAlign = 'left';
      break;
    case 'intro':
...
function init() {
  ...
  console.log('[init] hasSave:', G.hasSave, 'starting loop');
  // Iniciar loop
  loop();
}
